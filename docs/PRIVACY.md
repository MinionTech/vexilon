...
### 1. Accountability
*   **Designation**: The AgNav maintainer serves as the Privacy Officer.
...
*   **Purpose**: AgNav processes user queries for the sole purpose of providing context-aware answers in real-time. 
...
*   **Content-Blind**: AgNav does not collect user IP addresses (except for active session rate-limiting in memory), device fingerprints, or location data.
...
4.  **Closure** → Tab closes → RAM is cleared → No record remains on AgNav servers.
