...
The deployment process pushes a stub `Dockerfile` to the HF Space:

- **PR Preview (TEST):** Pull requests trigger [`.github/workflows/pr.yml`](.github/workflows/pr.yml), deploying to the `bcgeu/navigator-test` Space for preview and health checks.
- **PROD:** Merges to `main` trigger [`.github/workflows/merge.yml`](.github/workflows/merge.yml), deploying directly to the production `bcgeu/navigator` Space.
