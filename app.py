...
def get_system_prompt(developer_mode: bool = False) -> str:
    ...
    path = Path("./prompts/developer.txt") if developer_mode else Path("./prompts/steward.txt")
    if path.is_file():
        return path.read_text(encoding="utf-8")
    # Fallback to the original system prompt logic
    return "You are Vexilon, a professional assistant for BCGEU union stewards."
...
def build_index_from_sources(force: bool = False) -> None:
    """
    Parse source files, embed them, and build the index by calling scripts/build_index.py.
    """
    import subprocess
    ...
    print(f"[build] Calling external index builder: {' '.join(cmd)}")
    try:
        subprocess.run(cmd, check=True)
        print("[build] External indexing complete.")
    except subprocess.CalledProcessError as e:
        print(f"[build] Error: External indexing failed: {e}")
        sys.exit(1)
    ...
    try:
        async with client.messages.stream(
            ...
            system=[
                {
                    "type": "text",
                    "text": get_system_prompt(DEVELOPER_MODE).format(manifest=get_knowledge_manifest()),
