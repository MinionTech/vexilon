...
```
PR opened → pr.yml (test + build image + deploy to HF test Space)
          → merge to main → merge.yml (deploy to HF prod Space)
```
...
| File | Purpose |
|------|---------|
| `.github/workflows/pr.yml` | Tests + builds Docker image + deploys preview to test HF Space on PR |
| `.github/workflows/merge.yml` | Deploys to prod HF Space on merge to main |
